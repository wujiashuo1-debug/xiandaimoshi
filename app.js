(() => {
  ...
  const KEY = 'text_web_mining_quiz_v2';
  const OLD_V2 = 'text_web_mining_unused_v2';
  const OLD_V1_PROGRESS = 'text_web_mining_unused_progress';
  const OLD_V1_MISTAKES = 'text_web_mining_unused_mistakes';
  ...
  function exportData(output) {
    const payload = JSON.stringify({ exportedAt: new Date().toISOString(), app: 'text-web-mining-quiz-v2', data }, null, 2);
    if (output) output.value = payload;
    const blob = new Blob([payload], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `text-web-mining-quiz-progress-${new Date().toISOString().slice(0,10)}.json`;
